...
export default function NewProductForm({
  ...
  return (
    <div className={`relative ${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow p-6 mb-8`}>
      {submitting && (
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {typeof onClose === 'function' && (
          <button
            ...
            onClick={onClose}
            disabled={submitting}
            className={`p-2 rounded-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${isDarkMode ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-100'}`}
            title="Close"
          >
      ...
      <form onSubmit={onSubmit} className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                ...
                <>
                  ...
                  <div className="flex text-sm text-gray-600">
                    <label htmlFor="image" className={`relative cursor-pointer rounded-md font-medium ${isDarkMode ? 'text-pink-400 hover:text-pink-300' : 'text-pink-600 hover:text-pink-500'} focus-within:outline-none`}>
                      ...
                      <input id="image" name="image" type="file" className="sr-only" accept="image/*" onChange={onImageChange} disabled={submitting} />
                    </label>
        ...
        <div className="flex justify-end lg:col-span-3">
          <button type="submit" disabled={submitting} className={`inline-flex items-center px-6 py-3 text-base font-medium text-white border border-transparent rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500 ${submitting ? 'bg-pink-400 cursor-not-allowed' : 'bg-pink-600 hover:bg-pink-700'}`}>
