  ...
  export default function AdminPage() {
      ...
      fetch("http://localhost:5001/api/v1/orders", {
        ...
        .then((data) => {
          if (Array.isArray(data)) {
            ...
            console.error("Invalid response structure for orders:", data);
            setOrders([]);
          }
          setLoading(false);
        })
    ...
    const updateOrderStatus = (orderId, newStatus) => {
      const { token } = user;
      fetch(`http://localhost:5001/api/v1/orders/${orderId}`, {
        method: "PATCH",
        headers: {
    ...
    const filteredOrders = orders.filter(order => {
      ...
      // Using optional chaining to prevent errors if properties are missing
      return (
        (order.orderId || order._id)?.toString().toLowerCase().includes(query) ||
        (order.shippingAddress?.fullName || order.username || '').toLowerCase().includes(query) ||
    ...
    const sortedAndFilteredOrders = useMemo(() => {
      ...
      if (sortConfig?.key) {
        sortableItems.sort((a, b) => {
          ...
          const numericKeys = ['orderId', 'quantity', 'shippingFee', 'total'];
          ...
          <table className="w-full text-left">
            <thead className={`${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
              <tr>{[
                ...
                { key: 'products.0.name', label: 'Product' },
                { key: 'products.0.variation', label: 'Variation' },
                { key: 'products.0.quantity', label: 'Quantity' },
                { key: 'shippingAddress.state', label: 'Region' },
            ...
            <tbody className={`${isDarkMode ? 'divide-gray-700' : 'divide-gray-200'}`}>
              ...
              {orders.map((o) => (
                <tr key={o._id} className={isDarkMode ? 'hover:bg-gray-700/50' : 'hover:bg-gray-50'}>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm font-semibold ${isDarkMode ? 'text-pink-400' : 'text-pink-600'}`}>{o.orderId || o._id.substring(0, 8)}</td>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{o.shippingAddress?.fullName || o.username}</td>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{o.products?.[0]?.name}</td>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{o.products?.[0]?.variation}</td>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{o.products?.[0]?.quantity}</td>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{o.shippingAddress?.state}</td>
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>{o.shippingAddress?.city}</td>
                  ...
                  <td className={`px-6 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-900'}`}>₱{parseFloat(o.total || 0).toLocaleString()}</td>
      ...
      useEffect(() => {
        const fetchProducts = async () => {
          ...
          fetch('http://localhost:5001/api/v1/products', {
            headers: {
              // No token needed for public product fetching,
              // but if it were a protected route, you'd add:
