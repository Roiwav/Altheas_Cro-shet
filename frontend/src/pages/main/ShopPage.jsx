// src/pages/main/ShopPageClean.jsx
...
// Regions & Shipping (kept same values)
const shippingFees = {
  "Manila": 25,
  "Quezon City": 20,
  "Calamba City": 36,
  ...
  const handleBuyNow = () => {
    ...
    if (!isAuthenticated) {
      ...
      navigate("/signup", {
        state: {
          from: "shop-buy-now",
          product: { ...selectedProduct, variation: selectedVariation || "", quantity: modalQuantity },
        },
      });
      setSelectedProduct(null);
      return;
      ...
      <main className={`relative z-10 bg-gradient-to-b from-blue-50 to-white dark:from-gray-900 dark:to-gray-800 min-h-screen pt-16 pb-16 ${isAuthenticated ? 'px-6 md:px-20 lg:ml-[var(--sidebar-width,5rem)]' : ''} transition-all duration-300 ease-in-out`}>
        ...
        {!isAuthenticated && totalQuantity > 0 && (
          <div className="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => navigate("/signup", { state: { from: "shop-cart" } })} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors">Sign Up</button>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col flex-wrap items-center justify-between gap-4 mb-8 md:flex-row">
          <div className="flex items-center w-full gap-4 md:w-auto">
            <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="px-4 py-2 text-gray-900 transition-all bg-white border border-gray-300 rounded-full dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-pink-500 focus:border-transparent">
              {categories.map((cat) => (
                <option key={cat} value={cat}>{cat}</option>
              ))}
            </select>
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="px-4 py-2 text-gray-900 transition-all bg-white border border-gray-300 rounded-full dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-pink-500 focus:border-transparent">
          ...
          <div className="flex items-center justify-end w-full gap-4 md:w-auto">
            ...
            <div className="relative w-full sm:w-auto">
              ...
              <input type="text" placeholder="Search for flowers..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full px-4 py-2 pl-12 text-gray-900 transition-all bg-white border border-gray-300 rounded-full sm:w-80 dark:border-gray-600 dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
            </div>
          </div>
        </div>
            ...
            {paginatedProducts.map((product) => (
              ...
              >
                ...
                <button
                  ...
                  className="absolute z-10 p-2 transition-colors bg-white rounded-full shadow top-3 right-3 dark:bg-gray-900 hover:bg-pink-50"
                  aria-label={inWishlist(product.id) ? 'Remove from wishlist' : 'Add to wishlist'}
                >
                  <FaHeart className={`w-5 h-5 ${inWishlist(product.id) ? 'text-pink-600' : 'text-gray-300'}`} />
                </button>
        ...
        {paginatedProducts.length === 0 && searchQuery.length >= 3 && (
          <div className="py-20 text-center">
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-400">Your search for "{searchQuery}" did not match any products.</p>
          </div>
        )}
        ...
        {totalPages > 1 && (
          <div className="flex justify-center mt-16 md:justify-end">
            <div className="flex items-center gap-2">
              <button disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="px-4 py-2 transition-opacity bg-gray-200 rounded-full dark:bg-gray-700 dark:hover:bg-gray-600 hover:bg-gray-300 disabled:opacity-40">Previous</button>
              <span className="font-semibold text-gray-600 dark:text-gray-400">{page} / {totalPages}</span>
              <button disabled={page >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))} className="px-4 py-2 text-white transition-opacity bg-blue-500 rounded-full hover:bg-blue-600 disabled:opacity-40">Next</button>
            </div>
          </div>
        )}
      </main>
      ...
      <AnimatePresence>
        {selectedProduct && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div className="bg-white dark:bg-gray-800 rounded-xl overflow-hidden w-[95%] max-w-4xl shadow-2xl flex flex-col md:flex-row relative" initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}>
              {/* Modal heart */}
              <button onClick={() => handleToggleWishlist(selectedProduct)} className="absolute z-10 p-2 transition-colors bg-white rounded-full shadow top-3 left-3 dark:bg-gray-900 hover:bg-pink-50" aria-label={inWishlist(selectedProduct.id) ? 'Remove from wishlist' : 'Add to wishlist'}>
                <FaHeart className={`w-5 h-5 ${inWishlist(selectedProduct.id) ? 'text-pink-600' : 'text-gray-300'}`} />
              </button>
              ...
              <div className="flex flex-col justify-between p-6 space-y-6 bg-white md:w-1/2 dark:bg-gray-800">
                ...
                <div className="pt-4 space-y-4">
                  {selectedProduct.variations && selectedProduct.variations.length > 0 && (
                    <div>
                      <label htmlFor="variation-select" className="block mb-1 text-sm font-medium text-gray-700 dark:text-gray-300">Select Variation</label>
                      <select id="variation-select" value={selectedVariation} onChange={(e) => setSelectedVariation(e.target.value)} className="w-full px-3 py-2 text-gray-900 bg-white border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white focus:ring-2 focus:ring-pink-500">
                ...
                <div className="flex gap-4">
                  <button onClick={() => handleAddToCart(selectedProduct)} className="flex-1 py-3 text-lg font-semibold text-white transition bg-pink-600 rounded-lg hover:bg-pink-700">Add to Cart</button>
                  <button onClick={handleBuyNow} className={`flex-1 py-3 text-lg font-semibold transition border rounded-lg ${isAuthenticated ? 'text-red-600 bg-transparent border-red-600 hover:bg-red-600 hover:text-white dark:text-red-400 dark:border-red-400 dark:hover:bg-red-400 dark:hover:text-white' : 'text-orange-600 bg-transparent border-orange-600 hover:bg-orange-600 hover:text-white dark:text-orange-400 dark:border-orange-400 dark:hover:bg-orange-400 dark:hover:text-white'}`}>{isAuthenticated ? 'Buy Now' : 'Sign Up to Buy'}</button>
                </div>

                {!isAuthenticated && (
                  <div className="p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg">
                    <p className="text-orange-700 dark:text-orange-300 text-sm text-center">Create an account to buy items directly or continue as guest to add to cart!</p>
                  </div>
                )}
              </div>

              <button onClick={() => setSelectedProduct(null)} className="absolute text-2xl font-bold text-gray-400 top-3 right-3 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300">&times;</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
