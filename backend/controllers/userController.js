...
const updateUser = async (req, res) => {
  try {
    ...
    // IMPROVED: Smart password requirement logic
    let requirePassword = false;
    
    if (!isAdminUpdate) {
      // Check if this is just a deletion (addresses getting smaller)
      const isDeletion = addresses && 
                        Array.isArray(user.addresses) && 
                        Array.isArray(addresses) &&
                        addresses.length < user.addresses.length;

      // Check if this is adding new addresses
      const isAddingNew = addresses && 
                         (!user.addresses || addresses.length > user.addresses.length) ||
                         username !== undefined ||
                         avatar !== undefined ||
                         preferences !== undefined;

      // Only require password for additions/edits, not deletions
      requirePassword = isAddingNew && !isDeletion;

      console.log('🔐 Password requirement check:', {
        isDeletion,
        isAddingNew,
        requirePassword,
        currentAddressCount: user.addresses?.length || 0,
        newAddressCount: addresses?.length || 0
      });
    }
    ...
    if (addresses !== undefined) {
      console.log('📍 Updating addresses:', {
        oldAddresses: user.addresses,
        newAddresses: addresses,
        isDelete: addresses.length < (user.addresses?.length || 0)
      });
    ...
    console.log('✅ User updated successfully:', {
      ...
      addressCount: updatedUser.addresses?.length || 0,
      addresses: updatedUser.addresses
    });

    // Proper response format - FIXED JSON structure
    const response = {
      user: {
        id: updatedUser._id,
